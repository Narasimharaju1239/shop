          ...
          <div>
            ...
            {cartItems.map((item) => (
              <div key={`${item.product?._id || item._id}-${item.quantity}`} style={{
                ...
                <div style={{ flex: 1 }}>
                  <div>
                    {item.product?.name || item.name || '-'} <span style={{ color: '#888', fontSize: 14 }}>x {item.quantity}</span>
                  </div>
                ...
                <div>
                  ...
                  <button onClick={() => handleRemove(item)} style={{ padding: '5px 10px', background: '#e53935', color: '#fff', border: 'none', cursor: 'pointer', marginLeft: '10px' }}>Remove</button>
                </div>
              </div>
            ))}
          </div>
